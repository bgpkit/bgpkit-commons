...
use bgpkit_commons::{asinfo::AsInfoBuilder, BgpkitCommons};
use clap::{Parser, Subcommand};

#[derive(Parser)]
#[command(name = "bgpkit-export", version, about = "Export all bgpkit-commons data to Parquet")]
struct Cli {
...
fn write_asninfo_jsonl(
    ...
    let sorted: Vec<_> = all.keys().collect::<std::collections::BTreeSet<_>>().into_iter().collect();
    for asn in sorted {
